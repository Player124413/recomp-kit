...
В конфигурации `games/shrek2/game.toml` раскладка сенсорного геймпада и кнопок настроена в точности под официальное управление ПК-версии Shrek 2:

### Движение персонажа
- **Левый стик** / **D-Pad (крестовина)** — Стрелки клавиатуры `↑`, `↓`, `←`, `→`:
  - `↑` (Стрелка вверх) — Идти вперед
  - `↓` (Стрелка вниз) — Идти назад
  - `←` (Стрелка влево) — Идти влево
  - `→` (Стрелка вправо) — Идти вправо

### Действия и атака
- **Крест / ✕ (Cross)** или **L1 / L2** — `Левая кнопка мыши (ЛКМ)` / `Ctrl`:
  - Основной удар / Использовать предмет / Взять зелье
- **Круг / ○ (Circle)** — `Правая кнопка мыши (ПКМ)`:
  - Супер-удар (тратит шкалу героя)
- **Квадрат / □ (Square)** или **R1 / R2** — `Enter` (или `Shift`):
  - Прыжок
- **Треугольник / △ (Triangle)** — `Пробел (Space)`:
  - Защита / Блок (Шрек закрывается руками, Кот встает в стойку)
- **Правый стик** — Свободный обзор камеры (курсор мыши)

### Интерфейс и меню
- **Start** — `Esc`:
  - Пауза и выход в меню
- **Select** — `Tab`:
  - Посмотреть карту / Статистику уровня (монеты, плакаты)
- **Кнопка меню / PS** — Вызов настроек `recomp-kit` и наэкранного редактора кнопок

*Примечание:* Физические контроллеры (DualShock 4, DualSense, Xbox Controller, iPega) автоматически определяются через SDL3 и поддерживают виброотдачу.
