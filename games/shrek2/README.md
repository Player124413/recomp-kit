...
В конфигурации `games/shrek2/game.toml` уже предустановлена раскладка виртуального сенсорного геймпада:

| Кнопка геймпада | Действие в игре | Назначение |
|---|---|---|
| **Левый стик** | `WASD` | Передвижение персонажа |
| **Правый стик** | Мышь | Обзор камеры |
| **Крест / ✕ (Cross)** | Левая кнопка мыши | Атака / Удар |
| **Круг / ○ (Circle)** | Правая кнопка мыши | Действие / Диалог / Взаимодействие |
| **Квадрат / □ (Square)** | Пробел (`Space`) | Прыжок |
| **Треугольник / △ (Triangle)** | `Tab` | Смена персонажа (Шрек, Осёл, Кот, и др.) |
| **L2** | `LCtrl` | Присесть / Подкрасться |
| **R2** | `LShift` | Бег |
| **Start** | `Escape` | Меню паузы |
| **Select** | `F10` | Меню настроек и редактор наэкранных кнопок |
